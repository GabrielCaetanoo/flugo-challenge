...
export default function EditarColaborador() {
  ...
  // reset e navigate são referências instáveis — useRef evita que o useEffect re-dispare em loop
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setGestores(querySnapshot.docs.map(doc => ({ id: doc.id, name: doc.data().name })));
        setDepartamentos(depsSnapshot.map(d => ({ id: d.id, name: d.name })));
          ...
          <FormProvider {...methods}>
            <form onSubmit={handleSubmit(onSubmit)}>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, minHeight: 220 }}>
                {activeStep === 0 && (
                  <>
                    <TextField label="Nome" placeholder="João da Silva" variant="outlined" fullWidth InputLabelProps={{ shrink: true }} {...methods.register('name')} error={!!methods.formState.errors.name} helperText={methods.formState.errors.name?.message} sx={customInputStyle} />
