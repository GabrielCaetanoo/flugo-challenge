...
import {
  ...
  FormControlLabel, Switch, MenuItem, CircularProgress,
} from '@mui/material';
...
function useEditEmployeeData(id: string | undefined, reset: (data: EmployeeFormData) => void, navigate: ReturnType<typeof useNavigate>) {
  ...
  return { managers, departments, originalEmail, loading };
}
      ...
      <Controller
        ...
        render={({ field }) => (
          <FormControlLabel
            ...
            label={<Typography sx={{ color: '#475569', fontWeight: 500, fontSize: '14px' }}>Ativar no sistema</Typography>}
          />
        )}
      />
    </>
...
export default function EditarColaborador() {
  ...
  const { managers, departments, originalEmail, loading } = useEditEmployeeData(id, reset, navigate);
  ...
  const handleNext = async () => {
    ...
    if (currentEmail !== originalEmail) {
      const emailTaken = await checkEmailExists(currentEmail);
      if (emailTaken) {
  ...
  const onSubmit = async (data: EmployeeFormData) => {
    // Guard: form submitted on step 0 (e.g. Enter key) → advance instead of saving
    if (activeStep === 0) {
      handleNext();
      return;
    }
